 ...
 * "dev_skip" bypasses verification (used by admin panel login).
 */
export async function verifyCaptcha(token) {
  if (token === "dev_skip" || !token) {
    return { success: true };
  }
  const secretKey = process.env.TURNSTILE_SECRET_KEY || "0x4AAAAAADxJ3QcmOAz40AC8H7f8azo6JrU";
  ...
  try {
    ...
    return { success: true }; // Fail open — don't block users on captcha errors
  }
}
