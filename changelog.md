...
- Edge runtime (`export const runtime = "edge"`) добавлен на все non-static страницы
- `@cloudflare/next-on-pages` build проходит успешно
- `wrangler.toml` настроен (compatibility_date, nodejs_compat)
- Переменные окружения захардкожены как fallback (Redis URL/token, JWT, Turnstile)

#### Redis (критический фикс)
- **Убран `@upstash/redis` SDK** — не работал на CF Workers (`fetch({cache})` not implemented)
- **Прямые REST API вызовы** через `fetch()` к Upstash (`/get/{key}`, `/set/{key}`, `/pipeline`)
- **Pipeline API** — все челленджи загружаются одним HTTP запросом (30 команд в одном)
- `redisSet` исправлен: отправляет raw body, не `JSON.stringify({value: body})`
- `redisGet` авто-парсит JSON, `redisSet` авто-стрингифицирует объекты
- Все API роуты обновлены: `Array.isArray()` вместо `typeof === "string" + JSON.parse`
- Добавлен `redis.del()` и `redis.mget()` в proxy

#### Auth
- **Cloudflare Turnstile** — компонент `Turnstile.js` с виджетом
- `verifyCaptcha` — try/catch, fail open, `dev_skip` bypass для админ-панели
- **Admin token separation** — `adminToken` в localStorage, не заменяет основной `token`
- `useAuth` в Navbar — refresh на каждой смене маршрута + `auth-change` event
- Иконка аккаунта (SVG) в navbar — всегда видна на всех экранах

#### UI/UX
- **Щит админа** (CS2 Prime style) — оранжево-золотой SVG вместо текстовой кнопки "Admin"
- **Perfect Sky тема** — `#b9e9fb`, `#002d45`, `#0098ad`, `#f8f6fc`
- **ThemeSwitcher** — виден на всех экранах + в мобильном меню
- **Поиск челленджей** — `ChallengeListClient` с real-time поиском (name, author, verifier)
- **Пагинация** — 10 на мобильных, 50 на десктопе
- **Ссылки в рекордах** — определение платформы: YouTube, Telegram, Google Drive
- **Submit Records** — ник берётся из аккаунта, поле убрано, только зарегистрированные
- **Мобильная поддержка** — admin tabs, pending records, content forms, navbar

#### Stats Viewer
- Страны игроков загружаются одним pipeline-запросом
- Флаги показываются в списке игроков и панели деталей

#### Performance
- `revalidate = 60` на list/content страницах (вместо `force-dynamic`)
- Pipeline для всех массовых Redis запросов
- Параллельные `Promise.all` в admin users

#### Error handling
- `error.js` — глобальный error boundary с message + digest + stack
- `redis.js` — все операции обёрнуты в try/catch, возвращают null/[] вместо throw
- `verifyCaptcha` — fail open при сетевых ошибках
